...
def whan(n2, ha):
    ...
    for i in range(len(n2ha)):
        if np.isnan(n2ha[i])==True or np.isnan(wha[i])==True:
            clas.append(-1)
        else:
            if n2ha[i]>=-0.4 and wha[i]>=6:
                clas.append(4)
            elif n2ha[i]>=-0.4 and wha[i]<6 and wha[i]>=3:
                clas.append(3)
            elif n2ha[i]<-0.4 and wha[i]>3:
                clas.append(1)
            elif wha[i]<3:
                clas.append(0)
    return np.asarray(clas)
...
def cosmo(z, H0=70, WM=0.30, WV=0.70):
  '''
  convert angular diameter to kpc
  '''
...
def binormial_er(a, b, cl='cl'):
    ...
    for i in range(len(a)):
        ...
        bet = sp.special.betainc(k+1, n-k+1, z)
